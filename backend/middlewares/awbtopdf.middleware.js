    ...
    for (const awb of awbNumbers) {
      ...
      switch (conversionType) {
        ...
        case "Orange Label":
          ...
          break;
        case "Asendia":
          apiUrl = `${url}/asendiaTiff/${awb}/true`;
          break;
        default:
          apiUrl = "";
          console.error(
            `Unknown conversion type: ${conversionType} for AWB ${awb}`
          );
      ...
      try {
        ...
        if (!response.ok) {
          failedDownloads.push(awb);
          console.error(
            `Failed to fetch PDF content for AWB ${awb}: ${response.status} ${response.statusText}`
          );
          completedTasks++;
