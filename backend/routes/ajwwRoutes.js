...
const ajwwController = require('../manifestController/ajwwManifest.controller');

// AJWW Vendor Routes
// Route to generate Excel file from JSON data
router.post('/generate-excel', ajwwController.generateExcelFromJSON);

// Route to search for all bagging IDs by MAWB number
router.post('/search-baggings', ajwwController.searchBaggingsByMAWB);

// Route to search by MAWB number and generate Excel from database
router.post('/search-mawb', ajwwController.searchByMAWBAndGenerateExcel);

// Route to get JSON data
router.get('/json-data', ajwwController.getJSONData);

// Route to update JSON data
router.put('/update-json', ajwwController.updateJSONData);

// Route to download generated Excel file by MAWB number
router.get('/download/:mawbNumber', ajwwController.downloadExcelFile);
...
router.get('/files', async (req, res) => {
    try {
        ...
        const excelFiles = files.filter(file => 
            (file.startsWith('ajww_manifest_') || file.startsWith('mawb_')) && file.endsWith('.xlsx')
        );
